...
import com.quest.reward.repository.UserRepository;
...
public class RewardServiceImpl implements RewardService {
	...
	public List<RewardResponseDTO> getReward(LocalDate startDate, LocalDate endDate) {
		...
		for (PurchaseLogEntity purchaseLog : purchaseLogs) {
			...
			if (userRepository.findByTeamId(teamId).getStatus().equals("OK")) {
		...
		for (Map.Entry<Long, Long> entry : teamTotalAmounts.entrySet()) {
			...
			UserEntity user = userRepository.findByTeamId(teamId);
			...
			// 5만원 이상 10만원 미만
				...
				responseDTO.setReward(rewardList.get(5).getReward());
